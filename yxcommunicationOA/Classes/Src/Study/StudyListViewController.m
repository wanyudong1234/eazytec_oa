...
@interface StudyListViewController ()

@property (nonatomic,retain) EAUITableView *tableview;
@property (nonatomic,retain) NSMutableArray *studys;

@property (nonatomic,assign) int pageindex;
@property (nonatomic,assign) int pagesize;

@property (nonatomic,retain) NSString *studyno;
...
- (void)viewDidLoad {
    ...
    [_tableview registerClass:[StudyListTableViewCell class] forCellReuseIdentifier:@"StudyListTableViewCell"];
...
- (void)initData {
    ...
    _studys = [[NSMutableArray alloc]init];
    _pageindex = 1;
...
- (void)didLoadingDataWithPaging {
    ...
    [self didLoadingUserDataWithParamsNoError:dict andService:SERVICE_NAME_STUDY_LIST];
}
...
- (void)didAnalysisRequestResultWithData:(NSDictionary *)result andService:(REQUEST_SERVICE_NAME)name {
    ...
    [_studys addObjectsFromArray:[result objectForKey:@"List"]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return _studys.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    StudyListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"StudyListTableViewCell" forIndexPath:indexPath];
    
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    
    if (_studys.count < indexPath.row) {
        return cell;
    }
    cell.titlelabel.text = [_studys[indexPath.row] objectForKey:@"Title"];
    cell.timelabel.text  = [_studys[indexPath.row] objectForKey:@"Time"];
    return cell;
}

// 点击跳转事件
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    _studyno = [_studys[indexPath.row] objectForKey:@"Id"];
    [self performSegueWithIdentifier:@"StudyListToDetailSegue" sender:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    [segue.destinationViewController setValue:self.studyno forKey:@"studyno"];
}
