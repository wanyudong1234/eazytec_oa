...
#import "EASelectedAlertView.h"

/** 二进制码转RGB */
#define UIColorFromRGBValue(rgbValue) [UIColor colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0 green:((float)((rgbValue & 0xFF00) >> 8))/255.0 blue:((float)(rgbValue & 0xFF))/255.0 alpha:1.0]

@implementation EASelectedAlertView
/** 单例 */
+ (EASelectedAlertView *)singleClass{
    static EASelectedAlertView *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[EASelectedAlertView alloc] init];
    });
    return manager;
}
/** 提示view */
- (UIView *)quickAlertViewWithArray:(NSArray *)array{
    ...
    for (int i = 0; i < array.count;i++) {
        ...
        view.backgroundColor = COLOR_LABEL_GRAY;
        ...
        if ([array[i] isEqualToString:@"取消"]) {
            ...
            // 绿色背景
            view.backgroundColor = COLOR_LABEL_BLUE;
        }else{
